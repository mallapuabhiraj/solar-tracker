...
// --- Tuning Parameters ---
int servoPosition   = 90;   // Initial angle: centered (90°)
const int THRESHOLD = 10;   // Deadband — prevents jitter from minor light fluctuations
const int STEP      = 2;    // Degrees to move per control cycle

void setup() {
  trackerServo.attach(SERVO_PIN);
  trackerServo.write(servoPosition);
  Serial.begin(9600);
  Serial.println("Solar Tracker Initialized. Entering tracking loop...");
}

void loop() {
  int eastVal = analogRead(LDR_EAST);
  int westVal = analogRead(LDR_WEST);
  int delta   = eastVal - westVal;

  // Log to Serial Monitor / Serial Plotter for real-time visualization
  Serial.print("East: "); Serial.print(eastVal);
  Serial.print(" | West: "); Serial.print(westVal);
  Serial.print(" | Δ: "); Serial.print(delta);
  Serial.print(" | Angle: "); Serial.println(servoPosition);

  if (delta > THRESHOLD && servoPosition < 180) {
    servoPosition += STEP;        // Sun is brighter east → rotate east
  } else if (delta < -THRESHOLD && servoPosition > 0) {
    servoPosition -= STEP;        // Sun is brighter west → rotate west
  }
  // If |delta| ≤ THRESHOLD → panel is aligned, no movement
...
> 💡 **Pro tip:** Open **Arduino IDE → Tools → Serial Plotter** with this code running. You'll see a live real-time graph of both LDR values as the panel tracks — no extra code needed.
...
|---|-----------|:---:|-----------------|
| 1 | Arduino UNO R3 | 1 | ₹350 – ₹500 |
| 2 | SG90 Servo Motor | 1 | ₹120 – ₹180 |
| 3 | LDR / Photoresistor | 2 | ₹10 – ₹20 each |
| 4 | 10kΩ Resistor | 2 | ₹1 – ₹2 each |
| 5 | Mini Breadboard | 1 | ₹50 – ₹80 |
| 6 | Small Solar Panel (5V) | 1 | ₹150 – ₹300 |
| 7 | Jumper Wires (set) | 1 | ₹50 – ₹100 |
| | | **Total** | **~₹750 – ₹1,200** |

> 🛒 All components available at [Robu.in](https://robu.in), Amazon India, or any local electronics market.
